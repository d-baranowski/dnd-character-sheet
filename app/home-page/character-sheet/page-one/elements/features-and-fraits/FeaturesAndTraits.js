...
const FeaturesAndTraits = ({
  ...
  <g transform="translate(-250, 0)">
    ...
    <foreignObject x={540} y={430} width={240} height={520}>
      <div className={styles.featsContainer}>
        {pagedFeats.map(feat => (
          <div className={styles.featHeader} onClick={() => openModal(feat)}>
            {feat.name}
          </div>
        ))}
      </div>
    </foreignObject>
    <svg x={648} y={970} width={25} height={25} onClick={showEditor}>
...
const mapDispatchToProps = dispatch => ({
  showEditor: () =>
    dispatch(
      setDrawerRoute(FeaturesAndTraitsDrawerRoute, { hasSearchBar: true })
    ),
