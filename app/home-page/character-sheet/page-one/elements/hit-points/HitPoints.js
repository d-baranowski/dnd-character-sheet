...
class HitPoints extends React.PureComponent {
  ...
  getBarWidth = (maxHp, currentHp) =>
    Math.max(Math.min(currentHp / maxHp * 99, 99), 0);

  render() {
    const { isHovered } = this.state;
    const { currentHp } = this.props;
    const maxHp = this.props.tempMapHp
      ? this.props.tempMapHp
      : this.props.maxHp;
    ...
    return (
      <g>
        ...
        <path
          d="M 359 315 h 100 v 3 h -100 Z"
          fill="transparent"
          stroke="black"
        />
        <path
          style={{ transition: 'd 0.6s' }}
          d={`M 359.5 316.5 h ${this.getBarWidth(maxHp, currentHp)}`}
          stroke="red"
          strokeWidth={2}
        />
        <rect
