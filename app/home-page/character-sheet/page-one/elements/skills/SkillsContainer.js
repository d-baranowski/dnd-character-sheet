...
const getSkillValue = (
  skill,
  { attributes, skillModifiers, proficiencyBonus }
) =>
  parseInt(attributes[skill.attribute].modifier) +
  parseInt(skillModifiers[skill.name].bonus) +
...
const mapStateToProps = state => {
  ...
  const skillModifiers = skills.reduce((result, val) => {
    ...
    copy[val.name] = {};
    copy[val.name].bonus =
      getValue(state, 'skills', `${val.name}OtherBonus`) || 0;
    copy[val.name].proficient =
      getValue(state, 'skills', `${val.name}Proficient`) || 0;
    return copy;
  }, {});
