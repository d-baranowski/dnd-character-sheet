...
const getAttributeModifierValue = (state, attributeName) =>
  (state &&
    ...
    state.attributesReducer[`${attributeName}Modifier`]) ||
  0;
...
const getAttributeProficiency = (state, attributeName) =>
  (state &&
    ...
    state.form.savingThrows.values[`${attributeName}ThrowProficient`]) ||
  false;
...
const getOtherBonus = (state, attributeName) =>
  (state &&
    ...
    state.form.savingThrows &&
    state.form.savingThrows.values &&
    state.form.savingThrows.values[`${attributeName}OtherBonus`]) ||
  0;
...
const reducerMutator = reducer => (state, action) => {
  ...
  attributes.forEach(attributeName => {
    const attributeModifier = parseInt(
      getAttributeModifierValue(mutatedState, attributeName)
    );
    ...
    savingThrowsModifiers[attributeName] =
      attributeModifier +
      parseInt(otherBonus) +
