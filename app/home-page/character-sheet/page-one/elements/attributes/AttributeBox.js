...
class AttributeBox extends React.Component {
  ...
  onKeyPress = (event) => {
    ...
    if (key === "Backspace") {
      ...
      const newValue = insertCharacterAtPosition(this.state.value, this.state.cursor, key);
      if (!this.isValid(newValue)) {
        return
      }
      this.setState({
    ...
    return (
      <g transform={`translate(0 ${transformY})`}>
        ...
        <foreignObject x="47" y="247">
          <div>
            <p style={{height: '30px', width: '60px', border: 'none', fontFamily: 'Scala Sans Offc', fontSize: 'medium', textAlign: 'center' }}>{
