...
const AttacksModal = ({attack, modalOpen, closeModal, removeAttack, updateAttack}) => (
  <Modal
    open={modalOpen}
    onClose={closeModal}
  >
    ...
    <Modal.Content>
      <div>
        ...
        <Input
          ...
          value={attack.name}
          onChange={(event) => updateAttack({...attack, name: event.target.value})}
        />
      </div>
      <div>
        <h3>Attribute</h3>
      ...
      <FormCheckbox
        style={{marginBottom: 10}}
        input={{onChange: (value) => updateAttack({...attack, proficient: value})}}
      ...
      <div>
        ...
        <Input
          ...
          onChange={(event) => updateAttack({...attack, damageType: event.target.value})}/>
      </div>
      <div>
        <h3>Damage Dice</h3>
        <Input
      ...
      <div>
        ...
        <TextArea
          ...
          placeholder="Notes"
          value={attack.notes}
          onChange={(event) => updateAttack({...attack, notes: event.target.value})}
        />
      </div>
    </Modal.Content>
  </Modal>
