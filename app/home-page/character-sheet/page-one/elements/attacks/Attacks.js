...
const getAttackBonus = (attributeModifiers, attack, proficiencyBonus) =>
  parseInt(attributeModifiers[attack.attribute]) +
    (attack.proficient ? proficiencyBonus : 0) +
    parseInt(attack.otherBonus || '0') || 0;
...
const Attacks = ({
  ...
  <g transform="translate(250, 0)">
    ...
    <foreignObject x={297} y={522} width={300}>
      <svg x={400} y={760}>
        {Object.entries(attacks).map((value, index) => (
          <text
            ...
            fontFamily="Scala Sans Offc"
          >
            <tspan y={(index + 1) * 12} x="0">
              {value[1].name}
            </tspan>
            <tspan y={(index + 1) * 12} x={90}>
              {prependSign(
                getAttackBonus(attributeModifiers, value[1], proficiencyBonus)
              )}
            </tspan>
            <tspan y={(index + 1) * 12} x={138}>
              {value[1].damageDice}{' '}
              {prependSign(
                parseInt(attributeModifiers[value[1].attribute] || 0) +
                  parseInt(value[1].damageBonus || '0')
              )}{' '}
              / {value[1].damageType}
            </tspan>
          </text>
    ...
    <svg x={400} y={760} width={25} height={25} onClick={addAttack}>
