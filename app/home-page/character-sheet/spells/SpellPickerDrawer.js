...
class SpellsDrawer extends React.PureComponent {
  allPossibleValues = () =>
    spells.byClass[this.props.spellCastingClass][this.props.level].sort(
      (a, b) => {
  ...
  render() {
    const matchingSpells = [
      'New Spell',
      ...this.allPossibleValues().filter(spellName =>
        spellName.toUpperCase().includes(this.props.searchTerm.toUpperCase())
      )
    ];
...
const mapDispatchToProps = (dispatch, ownProps) => ({
  ...
  pickSpell: (spellName, level) => {
    dispatch(pickSpell(spellName, level, ownProps.spellCastingClass));
    dispatch(closeDrawer());
  }
});

export default drawerRoute(SpellsDrawerRoute)(
  connect(mapStateToProps, mapDispatchToProps)(SpellsDrawer)
);
