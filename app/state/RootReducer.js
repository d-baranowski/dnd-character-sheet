...
import savingThrowsMutator from '../home-page/character-sheet/page-one/elements/saving-throws/state/savingThrowsMutator';
...
const regainHitDice = combinedState => {
  ...
  const totalHitDice = calculateTotalLevel(
    combinedState.homePageReducer.classesReducer
  );
  const halfTotalRoundUp = Math.ceil(totalHitDice / 2);

  const hitDiceToRegain =
    halfTotalRoundUp < spendDice ? halfTotalRoundUp : spendDice;
  ...
  const dieArray = ['d20', 'd12', 'd10', 'd8', 'd6', 'd4'];
  ...
  for (let i = 0; i < dieArray.length; i++) {
    ...
    const currentDieValue =
      combinedState.homePageReducer.hitDiceReducer[dieArray[i]];
...
const regainSpellSlots = combinedState => {
  ...
  const expendedSpellSlots = Object.keys(combinedState.myFormReducer).filter(
    value => value.match(isSpellSlotsValue)
  );

  return expendedSpellSlots.reduce(
    (aggregator, value) => ({ ...aggregator, [value]: 0 }),
    {}
  );
};
...
const rootReducer = (state, action) => {
  ...
  if (action.type === 'LONG_REST') {
    return {
      ...
      myFormReducer: {
        ...combinedState.myFormReducer,
        ...regainSpellSlots(combinedState)
      }
    };
  }
