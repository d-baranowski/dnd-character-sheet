        ...
        {classesWithSpellsSlots.map((magicUser, index) => (
          <li
            ...
            style={{
              ...
              fill: 'white',
              stroke: 'black',
              zIndex: location === `/spells-of-${magicUser}` ? 100 : classesWithSpellsSlots.length - index + 3
            }}
          >
...
const mapStateToProps = (state) => {
  const classesWithSpellsSlots = Object.entries(state.homePageReducer.classesReducer.classes).filter(([key, value]) => {
    return value.level > 0 && Object.keys(spellSlotsPerClass).find(value => value === key);
  }).map(([key]) => key);
